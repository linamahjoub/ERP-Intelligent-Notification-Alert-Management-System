...
export default function EntrepotsScreen() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const list = await mobileCrudApi.getList('entrepots/');
      setWarehouses(list);
    } catch (error) {
      Alert.alert('Erreur', 'Impossible de charger les entrepots');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const filtered = useMemo(() => {
    ...
    return warehouses.filter((item) => {
      const fields = [item.name, item.code, item.city, item.country].map((v) => String(v || '').toLowerCase());
  ...
  const stats = useMemo(() => ({
    ...
    capacity: warehouses.reduce((sum, w) => sum + Number(w?.capacity || 0), 0),
  }), [warehouses]);
  ...
  const saveWarehouse = async () => {
    if (!formData.name.trim() || !formData.code.trim()) {
      Alert.alert('Validation', 'Nom et code sont requis');
      return;
    }
    ...
    try {
      if (formData.id) {
        await mobileCrudApi.update('entrepots', formData.id, payload);
      } else {
        await mobileCrudApi.create('entrepots/', payload);
      }
      setModalVisible(false);
      setFormData(emptyForm);
      fetchData();
    } catch (error) {
      Alert.alert('Erreur', 'Impossible d enregistrer l entrepot');
    }
  };
  ...
  const removeWarehouse = (item) => {
    Alert.alert('Confirmation', `Supprimer ${item.name} ?`, [
      { text: 'Annuler', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await mobileCrudApi.remove('entrepots', item.id);
            fetchData();
          } catch (error) {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.headerRow}>
        <Text style={styles.title}>Entrepots</Text>
      ...
      >
        <View style={styles.statsRow}>
          ...
          <View style={styles.statCard}><Text style={[styles.statValue, { color: '#8b5cf6' }]}>{stats.capacity}</Text><Text style={styles.statLabel}>Capacite</Text></View>
        </View>
            ...
            renderItem={({ item }) => (
              <View style={styles.itemCard}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.itemSub}>Code: {item.code} - {item.city || '-'}, {item.country || '-'}</Text>
                  <Text style={styles.itemSub}>Capacite: {item.capacity || 0}</Text>
                ...
                <View style={styles.actionsCol}>
                  ...
                  <TouchableOpacity onPress={() => removeWarehouse(item)} style={styles.iconBtn}>
                    <MaterialIcons name="delete" size={18} color="#ef4444" />
                  </TouchableOpacity>
                </View>
              </View>
            )}
            ListEmptyComponent={<Text style={styles.emptyText}>Aucun entrepot</Text>}
          />
        )}
      </ScrollView>
      ...
      <Modal visible={modalVisible} transparent animationType="slide" onRequestClose={() => setModalVisible(false)}>
        <View style={styles.modalOverlay}>
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalCard}>
            <Text style={styles.modalTitle}>{formData.id ? 'Modifier entrepot' : 'Nouvel entrepot'}</Text>
            ...
            <TextInput style={styles.input} value={formData.code} onChangeText={(value) => setFormData((prev) => ({ ...prev, code: value }))} placeholder="Code" placeholderTextColor="#64748b" />
            ...
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <TextInput style={[styles.input, { flex: 1 }]} value={formData.capacity} onChangeText={(value) => setFormData((prev) => ({ ...prev, capacity: value }))} placeholder="Capacite" placeholderTextColor="#64748b" keyboardType="numeric" />
              <TextInput style={[styles.input, { flex: 1 }]} value={formData.manager_name} onChangeText={(value) => setFormData((prev) => ({ ...prev, manager_name: value }))} placeholder="Manager" placeholderTextColor="#64748b" />
            </View>
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <TextInput style={[styles.input, { flex: 1 }]} value={formData.phone} onChangeText={(value) => setFormData((prev) => ({ ...prev, phone: value }))} placeholder="Telephone" placeholderTextColor="#64748b" keyboardType="phone-pad" />
              <TextInput style={[styles.input, { flex: 1 }]} value={formData.email} onChangeText={(value) => setFormData((prev) => ({ ...prev, email: value }))} placeholder="Email" placeholderTextColor="#64748b" keyboardType="email-address" />
            </View>

            <View style={styles.switchRow}>
              <Text style={styles.switchLabel}>Entrepot actif</Text>
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity style={styles.saveBtn} onPress={saveWarehouse}>
